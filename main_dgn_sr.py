...
from nerf.llff_provider import LlffDataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--type', type=str, default='llff', help="dataset type, supports (llff, blender)")
    ...
    model = NeRFNetwork(
        encoding="hashgrid",
        bound=opt.bound,
        cuda_ray=opt.cuda_ray,    # to create density field 
        density_scale=1,
    ...
    if opt.test:
        ...
        if opt.gui:
            ...
            if opt.type == 'blender':
                ...
                test_loader = LlffDataset(opt.path, device=device, split='test').dataloader()
        ...
        if opt.type == 'llff':
            train_loader = LlffDataset(opt.path, device=device, split='train').dataloader()
        elif opt.type == 'blender':
        ...
        if opt.gui:
            ...
            if opt.type == 'blender':
                ...
                valid_loader = LlffDataset(opt.path, device=device, split='val').dataloader()
